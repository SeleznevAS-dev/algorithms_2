...
def test_depth_first_search():
    ...
    result = simple_graph.DepthFirstSearch(0, 4)
    assert result == [simple_graph.vertex[0], simple_graph.vertex[1], simple_graph.vertex[4]]

    simple_graph.RemoveEdge(1, 4)
    result = simple_graph.DepthFirstSearch(0, 4)
    assert result == [simple_graph.vertex[0], simple_graph.vertex[1], simple_graph.vertex[3], simple_graph.vertex[4]]
    
    simple_graph.RemoveEdge(3, 4)
    result = simple_graph.DepthFirstSearch(0, 4)
    assert result == []
    
    result = simple_graph.DepthFirstSearch(0, 0)
    assert result == [simple_graph.vertex[0]]
    
    result = simple_graph.DepthFirstSearch(0, 5)
    assert result == []
    
    result = simple_graph.DepthFirstSearch(0,2)
